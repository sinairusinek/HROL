...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="placeName">
        <span class="place" style="background-color: #ffffcc"><xsl:apply-templates/></span>
    </xsl:template>
    <xsl:template match="persName">
        <span class="person" style="background-color: #ffaacc"><xsl:apply-templates/></span>
    </xsl:template>
    
    
    <xsl:template match="persName" mode="toc">
        <li>
            <xsl:apply-templates/>
        </li>
    </xsl:template>  
